...
TotalCost = sum {j in TECHNOLOGIES diff RENOVATION} (tau [j] * ( C_inv [j] - R_gc [j] - R_inc [j] ) + C_maint [j] ) + sum {k in RENOVATION} ( tau [k] * (C_inv [k] - R_ren [k]) ) + sum {i in RESOURCES} C_op [i] /*+ Prosumer_tax*/; 
...
/*subject to capacity_factor_t_HP {hp in HPS, h in HOURS, td in TYPICAL_DAYS}:
	F_t [hp, h, td] / cop_time_series[hp,"HEAT_LOW_T_DECEN", h, td] <= F [hp] * c_p_t [hp, h, td];*/
...
subject to peak_lowT_dec {j in TECHNOLOGIES_OF_END_USES_TYPE["HEAT_LOW_T_DECEN"] diff {"DEC_SOLAR"}, h in HOURS, td in TYPICAL_DAYS}:
	F [j] >= peak_sh_factor * peak_real_factor * F_t [j, h, td];

/*subject to peak_lowT_dec_HP {hp in HPS, h in HOURS, td in TYPICAL_DAYS}:
	F [hp] >= peak_sh_factor * F_t [hp, h, td] /** cop_time_series[hp,"ELECTRICITY",h,td]*/;
