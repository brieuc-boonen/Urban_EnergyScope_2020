...
for{{0}: sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(F_t ["CSS_COLL", h, td]  ) > 10}{
	printf "%s,%s,%.2f,%s,%s,%s\n", "Solar" , "CCS Heat", sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(layers_in_out["CSS_COLL","CSS_HEAT"] * F_t ["CSS_COLL", h, td]  ) / 1000 , "Solar", "#FFFF00", "TWh" >> "output/sankey/input2sankey.csv";
}
...
# CSS_HEAT use
#------------------------------------------
for{{0}: sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(F_t ["CSS_ST", h, td]  ) > 10}{
	printf "%s,%s,%.2f,%s,%s,%s\n", "CCS Heat" , "Elec", sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(-layers_in_out["CSS_ST","CSS_HEAT"] * F_t ["CSS_ST", h, td]  ) / 1000 , "CSS Heat", "#DC143C", "TWh" >> "output/sankey/input2sankey.csv";
}
#for{{0}: sum{i in STORAGE_OF_END_USES_TYPES["CSS_HEAT"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(Storage_in [i, "CSS_HEAT", h, td]) > 10}{
#	printf "%s,%s,%.2f,%s,%s,%s\n", "CSS Heat" , "CSS Storage", sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (sum {i in STORAGE_OF_END_USES_TYPES["CSS Heat"] }(Storage_in [i, "CSS_HEAT", h, td]))/ 1000 , "CSS Heat", "#00BFFF", "TWh" >> "output/sankey/input2sankey.csv";
#}
#for{{0}: sum{i in STORAGE_OF_END_USES_TYPES["CSS_HEAT"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(Storage_in [i, "CSS_HEAT", h, td]) > 10}{
#	printf "%s,%s,%.2f,%s,%s,%s\n", "CSS Storage" , "CSS Heat", sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(sum {i in STORAGE_OF_END_USES_TYPES["ELECTRICITY"] } (Storage_out [i, "CSS_HEAT", h, td] / storage_eff_out [i, "ELECTRICITY"])  )/ 1000
#	, "CSS Heat", "#00BFFF", "TWh" >> "output/sankey/input2sankey.csv";
#}
#for{{0}: sum{i in STORAGE_OF_END_USES_TYPES["ELECTRICITY"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}(Storage_in [i, "CSS_HEAT", h, td]) > 10}{
#	printf "%s,%s,%.2f,%s,%s,%s\n", "CSS Storage" , "Exp & Loss", sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (sum {i in STORAGE_OF_END_USES_TYPES["ELECTRICITY"] }((Storage_in [i, "CSS_HEAT", h, td])- (Storage_out [i, "ELECTRICITY", h, td] / storage_eff_out [i, "ELECTRICITY"]))  )/ 1000 , "CSS Heat", "#00BFFF", "TWh" >> "output/sankey/input2sankey.csv";
#}
