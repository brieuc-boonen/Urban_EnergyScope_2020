...
subject to rinc_cost_calc {j in TECHNOLOGIES}:
	R_inc [j] = (c_inc_fix [j] + c_inc_var [j] * living_area) * X_active [j] / lifetime [j];#sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} ( F_t [j, h, td] * t_op [h, td] );
